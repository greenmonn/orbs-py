...
criteria_text = r'Criteria Match'
component_path = 'intermediate.html'
test_path = 'match.spec.js'
...
class Path():
    def __init__(self, target_file_path):
        ...
        self.temp_file_name = '{}_orbs.html'.format(file_name[:-5])
...
class ORBSlicer():
    ...
    def setup(self, filepath, arguments):
        ...
        code_str = code_str.replace(
            '{}.html'.format(component_name), '{}_orbs.html'.format(component_name))
    ...
    def _execute_program(self):
        ...
        argv = ['babel-node', self.test_filepath]
        ...
        if len(matches) > 0:
            print('✔️ Criteria Met!')
            return True

        print('🚫 Not Match - Criteria Broken!')
        return False
    ...
    def _compile_program(self):
        ...
        code_str = ''.join(self.program_lines)
        try:
            _ = compile(code_str, 'orbs_target.py', 'exec')
    ...
    def do_slicing(self):
        ...
        # record original visual semantics + related behaviors
        ...
        while not deleted:  # until nothing can be deleted
            ...
            while i < len(self.program_lines):
                ...
                for j in range(0, self.MAX_DEL_WINDOW_SIZE):
                    ...
                    builds = True
                    if builds:
                        # TODO: syntax check for HTML (or at least javascript)?
                        # currently just one-line deletions (costly)
                ...
                if builds:
                    ...
                    if execute_success:
                        ...
                        self._write_code_to_file(
                            os.path.join(self.path.get_result_path(), 'success/success_{}_{}.html'.format(
                                i, success_count)))
                        success_count += 1

                    else:
                        self._write_code_to_file(
                            os.path.join(self.path.get_result_path(), 'execfail/execfail_{}_{}.html'.format(
                                i, execfail_count)))
                        execfail_count += 1
                    ...
                    self._write_code_to_file(
                        os.path.join(self.path.get_result_path(), 'buildfail/buildfail_{}_{}.html'.format(
                            i, buildfail_count)))
                    buildfail_count += 1
...
def main():
    ...
    filepath, arguments = test_path, []

    if len(argv) > 0:
        filepath = argv[0]
        arguments = argv[1:]

    print('Target Web Page: ', component_path)
